...
import {
    storeFacultyCapstone, uploadFacultyResource, uploadFacultyImrad,
...
export default function CapstoneAdditionalInfoPage() {
    ...
    useEffect(() => {
        if (!initialForm || !pdfInfo) navigate('/faculty/capstone-library', { replace: true });
    }, []);
    ...
    const handleImradSelect = async (file) => {
        ...
        try {
            ...
            fd.append('file', file);
            const res = await uploadFacultyImrad(fd);
    ...
    const handleAddResource = async () => {
        ...
        try {
            ...
            const res = await uploadFacultyResource(fd);
            const { file_path, file_original_name } = res.data.data;
        ...
        <div className="min-h-screen flex flex-col" style={{ background: 'var(--color-bg)' }}>
            ...
            <div className="flex-1 overflow-y-auto admin-scroll pb-28">
                <div className="max-w-6xl mx-auto px-4 lg:px-8 py-6 space-y-5">
                        ...
                        <SectionCard icon={HiOutlineLink} title="Additional Resources" subtitle="Attach supplementary files" accent="#2563eb">
                                    ...
                                    {resources.map((r) => (
                                        <div key={r.id} className="flex items-center gap-2 p-2.5 rounded-lg border"
                                            ...
                                            <button onClick={() => setResources(prev => prev.filter(x => x.id !== r.id))}
                                                className="p-1 rounded transition-colors flex-shrink-0" style={{ color: 'var(--color-text-muted)' }}
                                                onMouseEnter={e => e.currentTarget.style.color = '#ef4444'}
                                                onMouseLeave={e => e.currentTarget.style.color = 'var(--color-text-muted)'}>
                            ...
                            <div className="rounded-xl border-2 border-dashed p-3 space-y-2.5" style={{ borderColor: 'var(--color-border-strong)' }}>
                                <input type="text" value={newResourceName} onChange={(e) => setNewResourceName(e.target.value)}
                                    placeholder="Resource name (e.g. Source Code, Dataset)…"
                                    className={inputCls} />
                                <div onClick={() => resourceFileRef.current?.click()}
                                    ...
                                    <HiOutlineUpload className="w-4 h-4 flex-shrink-0" style={{ color: 'var(--color-text-muted)' }} />
                            ...
                            <div className="relative">
                                <div className="relative">
                                    ...
                                    <input type="text" value={refSearch}
                                        ...
                                        placeholder="Search by title to cite…"
                                            ...
                                            <div className="max-h-48 overflow-y-auto admin-scroll divide-y" style={{ borderColor: 'var(--color-border)' }}>
                                                {refResults.map((cap) => (
                                                    <button key={cap.id} onClick={() => addReference(cap)}
                                                        ...
                                                        <p className="text-xs mt-0.5" style={{ color: 'var(--color-text-muted)' }}>{cap.author}{cap.year && <span className="mx-1">·</span>}{cap.year}</p>
                                                    </button>
                                                ))}
                                            </div>
                                        )}
                    ...
                    <div className="grid grid-cols-1 lg:grid-cols-2 gap-5">
                                                ...
                                                <div className="max-h-64 overflow-y-auto admin-scroll divide-y" style={{ borderColor: 'var(--color-border)' }}>
                                                    {adviserResults.filter(f => !adviserSearch || f.name.toLowerCase().includes(adviserSearch.toLowerCase())).map((fac) => (
                                                        <button key={fac.id}
                                                            ...
                                                            onMouseEnter={e => e.currentTarget.style.background = 'var(--color-bg-tertiary)'}
                                                            onMouseLeave={e => e.currentTarget.style.background = 'transparent'}>
                                                            <div className="w-7 h-7 rounded-full flex items-center justify-center flex-shrink-0" style={{ background: 'var(--color-bg-tertiary)' }}>
                        ...
                        {/* Right half — summary info card */}
                        <div className="rounded-2xl border p-5 flex flex-col justify-between" style={{ background: 'var(--color-surface)', borderColor: 'var(--color-border)' }}>
                            <div>
                                ...
                                <div className="space-y-2.5">
                                    ...
                                    <div className="flex items-center justify-between">
                                        <span className="text-xs" style={{ color: 'var(--color-text-muted)' }}>Copyright</span>
                                        {currentCopyright
                                    ...
                                    <div className="flex items-center justify-between">
                                        <span className="text-xs" style={{ color: 'var(--color-text-muted)' }}>IMRAD</span>
                                        <span className="text-xs font-semibold" style={{ color: 'var(--color-text)' }}>{imradInfo ? '✓ Uploaded' : 'Not uploaded'}</span>
                                    </div>
                                    <div className="flex items-center justify-between">
                                        <span className="text-xs" style={{ color: 'var(--color-text-muted)' }}>Resources</span>
                                        <span className="text-xs font-semibold" style={{ color: 'var(--color-text)' }}>{resources.length} file{resources.length !== 1 ? 's' : ''}</span>
                                    </div>
                                    <div className="flex items-center justify-between">
                                        <span className="text-xs" style={{ color: 'var(--color-text-muted)' }}>References</span>
                                        <span className="text-xs font-semibold" style={{ color: 'var(--color-text)' }}>{references.length} capstone{references.length !== 1 ? 's' : ''}</span>
                                    </div>
                                    <div className="flex items-center justify-between">
                                        <span className="text-xs" style={{ color: 'var(--color-text-muted)' }}>Adviser</span>
                                        <span className="text-xs font-semibold" style={{ color: 'var(--color-text)' }}>{adviser ? adviser.name : '—'}</span>
                                    </div>
                                </div>
                            </div>
                        </div>
            ...
            <div className="fixed bottom-0 left-0 right-0 z-30 border-t"
                ...
                <div className="max-w-6xl mx-auto px-4 lg:px-8 py-4 flex items-center justify-between gap-4">
                    ...
                    <button id="save-capstone-btn" onClick={handleSave} disabled={saving}
