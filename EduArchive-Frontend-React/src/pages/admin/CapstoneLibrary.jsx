...
export default function CapstoneLibrary() {
    ...
    const handleArchive = (capstone) => {
        setConfirm({
            ...
            message: `Archive "${capstone.title}"? You can restore it later from the Archive folder.`,
            variant: 'warning',
            action: async () => {
                try {
                    await archiveCapstone(capstone.id);
    ...
    const handleUnarchive = (capstone) => {
        setConfirm({
            ...
            action: async () => {
                try {
                    await unarchiveCapstone(capstone.id);
                        ...
                        <div className="flex items-center gap-3">
                            ...
                            <div>
                                ...
                                <p className="text-sm text-gray-500 mt-2">
                                    {viewing === 'archived' ? 'Manage archived capstone projects' : 'Manage all capstone projects'}
                                </p>
                            </div>
                        </div>
                        <div className="flex items-center gap-3">
                            ...
                            {viewing === 'active' && (
                                <button
                                    ...
                                    className="inline-flex items-center gap-1.5 px-4 py-2.5 text-sm font-medium text-white bg-green-600 border border-green-600 rounded-lg hover:bg-green-700 transition-colors shadow-sm"
                                >
                ...
                <div className="px-4 lg:px-8 py-6">
                    {loading ? (
                        ...
                        <EmptyState
                            ...
                            description={debouncedSearch || filters.year || filters.program || filters.category ? 'Try adjusting your search or filters.' : 'Upload capstones to get started.'}
                                        ...
                                        {capstones.map((cap, idx) => (
                                            <tr key={cap.id} className={`border-b border-gray-100 hover:bg-gray-50 transition-colors ${idx % 2 === 0 ? 'bg-white' : 'bg-gray-50'}`}>
                                                <td className="py-3 px-4 text-sm">
                                                    ...
                                                    <div className="text-xs text-gray-500">{cap.program || '—'} • {cap.year || '—'}</div>
                                                </td>
