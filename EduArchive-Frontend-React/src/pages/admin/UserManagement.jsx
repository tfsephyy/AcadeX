...
import { HiOutlineUserAdd, HiOutlineAcademicCap, HiOutlineBriefcase, HiOutlineEye, HiOutlineCheck, HiOutlineX, HiOutlineTrash, HiOutlineSearch } from 'react-icons/hi';
import { HiOutlineArchiveBoxArrowDown, HiOutlineArchiveBoxXMark } from 'react-icons/hi2';
import { getNewUsers, getStudents, getFaculty, approveUser, denyUser, removeUser, archiveUser, unarchiveUser, getArchivedUsers } from '../../api/admin';
...
export default function UserManagement() {
    ...
    const fetchTotals = async () => {
        try {
            const [newRes, stuRes, facRes, archRes] = await Promise.all([
    ...
    const currentList = activeTab === 'new' ? newUsers : activeTab === 'students' ? students : activeTab === 'faculty' ? faculty : archivedList;
    ...
    return (
        <div className="flex flex-col h-full">
            ...
            {loading ? (
                ...
                <EmptyState
                    title={activeTab === 'new' ? 'No new registrations' : activeTab === 'students' ? 'No students' : activeTab === 'faculty' ? 'No faculty' : 'No archived users'}
                    description={debouncedSearch || roleFilter || programFilter ? 'Try adjusting your search or filter.' : (activeTab === 'new' ? 'New user registrations will appear here.' : activeTab === 'students' ? 'Approved students will appear here.' : activeTab === 'faculty' ? 'Approved faculty members will appear here.' : 'Archived users will appear here.')}
                    icon={activeTab === 'archived' ? <HiOutlineArchiveBoxArrowDown className="w-12 h-12" /> : activeTab === 'new' ? <HiOutlineUserAdd className="w-12 h-12" /> : activeTab === 'students' ? <HiOutlineAcademicCap className="w-12 h-12" /> : <HiOutlineBriefcase className="w-12 h-12" />}
                />
            ) : (
                    ...
                    <div className="space-y-4">
                        <div className="flex items-center gap-4">
                            ...
                            <div>
                                ...
                                <p className="text-sm text-gray-500 capitalize">{viewUser.role?.name}</p>
                            </div>
                        </div>
